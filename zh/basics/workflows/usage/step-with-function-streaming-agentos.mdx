---
title: 在 AgentOS 上使用自定义函数流式传输的步骤
description: 此示例演示了如何在 AgentOS 上使用带有自定义函数执行器和流式传输的命名步骤。
---

此示例演示了如何将 Step 对象与自定义函数执行器结合使用，以及如何使用 [AgentOS](/agent-os/introduction) 来流式传输其响应。

在自定义函数步骤内部运行的智能体和团队也可以直接将其结果流式传输到 AgentOS。
