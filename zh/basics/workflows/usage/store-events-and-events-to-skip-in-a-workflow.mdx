---
title: 在工作流中存储事件和跳过事件
description: 本示例演示了 **Workflows 2.0** 的事件存储功能。
---

本示例演示了 **Workflows 2.0** 的事件存储功能，展示了如何：
1. **存储执行事件**以供调试/审计使用 (`store_events=True`)
2. **过滤冗余事件** (`events_to_skip`)，以便专注于关键的工作流里程碑
3. 通过 `workflow.run_response.events` 在执行后**访问已存储的事件**

## 主要特性：
- **选择性存储**：跳过冗长的事件（例如 `step_started`），同时保留关键里程碑。
- **调试/审计**：捕获执行流以供分析，无需手动记录日志。
