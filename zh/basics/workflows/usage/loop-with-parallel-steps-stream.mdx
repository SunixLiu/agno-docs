---
title: 带并行步骤的循环工作流
description: 本示例演示了 **Workflows 2.0** 最复杂的模式，该模式结合了循环执行、并行处理和实时流式传输。
---

本示例展示了如何创建迭代工作流，使其在每次迭代中同时执行多个独立任务，从而优化质量和性能。

**何时使用**：当您需要在每次迭代中通过并行执行任务来实现质量迭代提升时。非常适合用于综合研究工作流，即多个独立任务共同促成整体质量，并且需要重复执行直到满足质量阈值。
...
这是该模式的同步流式传输示例。要查看异步和非流式传输版本，请参阅Cookbook-
- [带并行步骤的循环工作流 (同步)](https://github.com/agno-agi/agno/tree/main/cookbook/05_workflows/_03_workflows_loop_execution/sync/loop_with_parallel_steps.py)
- [带并行步骤的循环工作流 (异步非流式传输)](https://github.com/agno-agi/agno/blob/main/cookbook/05_workflows/_03_workflows_loop_execution/sync/loop_with_parallel_steps.py)
- [带并行步骤的循环工作流 (异步流式传输)](https://github.com/agno-agi/agno/tree/main/cookbook/05_workflows/_03_workflows_loop_execution/async/loop_with_parallel_steps_stream.py)