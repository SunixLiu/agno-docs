---
title: 带输入模式验证的工作流
description: 本示例演示了 **Workflows** 如何使用 Pydantic 模型支持输入模式验证，以确保工作流入口处的类型安全和数据完整性。
---

本示例展示了如何在工作流中使用输入模式验证来强制执行类型安全和数据结构验证。通过使用 Pydantic 模型定义 `input_schema`，您可以确保工作流在开始执行前接收到经过正确结构化和验证的数据。

## 主要特性：
- **类型安全**：针对 Pydantic 模型自动验证输入数据
- **结构验证**：确保所有必填字段都存在且类型正确
- **明确的契约**：精确定义您的工作流期望的数据
- **预防错误**：在工作流执行开始前捕获无效输入
- **多种输入格式**：支持 Pydantic 模型和匹配的字典
