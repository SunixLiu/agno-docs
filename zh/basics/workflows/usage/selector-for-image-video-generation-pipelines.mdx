---
title: 用于图像/视频生成管道的选择器
description: 本示例演示了 **Workflows 2.0** 的路由器模式，该模式用于在图像和视频生成管道之间进行动态选择。
---

本示例演示了 **Workflows 2.0** 的路由器模式，用于在图像和视频生成管道之间进行动态选择。它使用 `Steps` 来封装每种媒体类型的工作流，并使用 `Router` 根据输入分析智能地选择管道。

## 主要特性：
- **动态路由**：根据输入关键词（例如“image”或“video”）选择管道（`Steps`）。
- **模块化管道**：将图像/视频工作流封装为可重用的 `Steps` 对象。
- **结构化输入**：使用 Pydantic 模型进行类型安全的配置（例如分辨率、风格）。

## 主要特性：
- **嵌套逻辑**：在 `Steps` 序列中嵌入 `Condition` 和 `Parallel`。
- **主题专项研究**：使用 `Condition` 为科技主题触发并行的科技/新闻研究。
- **模块化设计**：将整个工作流封装为可重用的 `Steps` 对象。
