---
title: 带循环步骤的路由器工作流
description: 本示例演示了 **Workflows 2.0** 的高级模式，该模式结合了基于路由器的智能路径选择与循环执行，以实现迭代式的质量提升。
---

本示例展示了如何创建自适应工作流，使其能够根据主题的复杂性选择最佳的研究策略和执行模式。

**何时使用**：当不同的主题类型需要截然不同的研究方法时——有些需要简单的单次研究，而另一些则需要迭代式的深度分析。非常适合用于内容自适应的工作流，即处理的复杂性应与内容的复杂性相匹配。
...
要查看异步版本，请参阅Cookbook-
- [带循环步骤的路由器工作流 (异步)](https://github.com/agno-agi/agno/tree/main/cookbook/05_workflows/_05_workflows_conditional_branching/async/router_with_loop_steps.py)