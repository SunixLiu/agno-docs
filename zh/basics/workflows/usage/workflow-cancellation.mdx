---
title: 工作流取消
description: 本示例演示了 **Workflows 2.0** 对取消正在运行的工作流执行的支持，包括基于线程的取消和处理已取消的响应。
---

本示例展示了如何实时取消正在运行的工作流执行。它演示了：

1. **基于线程的执行**：在单独的线程中运行工作流以实现非阻塞操作
2. **动态取消**：在工作流活跃运行时取消它们
3. **取消事件**：处理并响应取消事件
4. **状态跟踪**：在整个执行和取消过程中监控工作流状态
