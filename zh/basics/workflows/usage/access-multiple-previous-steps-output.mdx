---
title: 访问多个前序步骤的输出
description: 本示例演示了 **Workflows 2.0** 的高级数据流功能。
---

本示例演示了 **Workflows 2.0** 的以下功能：
1. 访问**特定命名步骤**的输出 (`get_step_content()`)
2. 聚合**所有前序步骤**的输出 (`get_all_previous_content()`)
3. 通过组合多个研究来源创建综合报告

## 主要特性：
- **步骤输出访问**：按名称或集合方式检索任何前序步骤的数据。
- **自定义报告**：组合并分析来自并行或顺序步骤的输出。
- **流式传输支持**：执行期间的实时更新。
