---
title: 每个步骤级别的结构化 I/O
description: 本示例演示了 **Workflows 2.0** 在智能体/团队/自定义 Python 函数之间类型安全的数据流。
---

展示了 **Workflows 2.0** 在智能体/团队/自定义 Python 函数之间类型安全的数据流。每个步骤：
1. 接收结构化（Pydantic 模型、列表、字典或原始字符串）输入
2. 产生结构化输出（例如 `ResearchFindings`, `ContentStrategy`）

您也可以使用此模式创建可在任何步骤中使用的自定义函数，您可以：
1. 检查传入的数据类型（原始字符串或 Pydantic 模型）。
2. 分析来自前序步骤的结构化输出。
3. 在保持类型安全的同时生成报告。
...
此模式适用于更多场景的示例：
- [团队在每个步骤级别的结构化 IO](https://github.com/agno-agi/agno/blob/main/cookbook/05_workflows/_06_advanced_basics/_01_structured_io_at_each_level/structured_io_at_each_level_team_stream.py)
- [自定义函数在每个步骤级别的结构化 IO-1](https://github.com/agno-agi/agno/blob/main/cookbook/05_workflows/_06_advanced_basics/_01_structured_io_at_each_level/structured_io_at_each_level_function_1.py)
- [自定义函数在每个步骤级别的结构化 IO-2](https://github.com/agno-agi/agno/blob/main/cookbook/05_workflows/_06_advanced_basics/_01_structured_io_at_each_level/structured_io_at_each_level_function_2.py)