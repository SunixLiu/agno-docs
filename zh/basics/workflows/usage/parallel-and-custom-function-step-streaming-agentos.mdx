---
title: AgentOS 上的并行与自定义函数步骤流式传输
description: 本示例演示了如何在 AgentOS 上使用自定义函数执行器和流式传输的并行步骤。
---

本示例演示了如何将步骤与自定义函数执行器结合使用，以及如何使用 [AgentOS](/zh/agent-os/introduction) 流式传输其响应。

在 `Parallel` 中的自定义函数步骤内运行的智能体（agents）和团队（teams）也将其结果流式传输到 AgentOS。
