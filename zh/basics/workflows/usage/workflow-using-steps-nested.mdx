---
title: 使用嵌套模式的 Steps 工作流
description: 此示例演示了 **Workflows 2.0** 如何使用 `Steps` 通过条件并行执行来封装复杂的工作流。
---

此示例演示了**工作流**如何利用嵌套模式和 `Steps`，通过条件并行执行来封装复杂的工作流。它结合了 `Condition`（条件）、`Parallel`（并行）和 `Steps`，以实现模块化和自适应的内容创建。
