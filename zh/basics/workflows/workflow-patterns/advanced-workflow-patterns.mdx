---
title: 高级工作流模式
description: 组合多种工作流模式，构建复杂且可用于生产的自动化系统
---

**模式组合**：条件逻辑 + 并行执行 + 迭代循环 + 自定义处理 + 动态路由

本示例演示了如何组合确定性模式，以创建复杂但可预测的工作流。
...
- [条件与并行步骤（流式示例）](/zh/basics/workflows/usage/condition-and-parallel-steps-stream)
- [包含并行步骤的循环（流式示例）](/zh/basics/workflows/usage/loop-with-parallel-steps-stream)
- [包含循环步骤的路由](/zh/basics/workflows/usage/router-steps-workflow)
