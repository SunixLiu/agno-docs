...
此示例演示了如何在团队中使用 `input_schema` 实现自动输入验证和结构化数据处理，从而自动验证字典形式的输入并将其转换为 Pydantic 模型。

## 代码

```python cookbook/02_examples/teams/structured_input_output/06_input_schema_on_team.py
"""
