...
keywords: [sessions, agent sessions, multi-turn conversations]
---
...
See [Session Storage](/concepts/db/overview) for more details on how sessions are stored.

## Single session

Here we have an example where a single run is created with an Agent. A `run_id` is automatically generated, as well as a `session_id`.
